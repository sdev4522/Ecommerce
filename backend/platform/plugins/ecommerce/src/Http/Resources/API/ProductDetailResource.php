...
class ProductDetailResource extends AvailableProductResource
{
    public function toArray($request): array
    {
        ...
        return [
            ...
            'specifications' => method_exists($this->resource, 'getVisibleSpecificationAttributes')
                ? $this->resource->getVisibleSpecificationAttributes()->map(function ($attribute) {
                    return [
                        'name' => $attribute->name,
                        'value' => (string) ($attribute->pivot?->value ?? ''),
                    ];
                })->filter(fn($item) => !empty($item['value']))->values()->all()
                : [],
        ];
    }
}
