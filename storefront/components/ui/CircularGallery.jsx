...
const DEFAULT_FONT = '600 20px Inter, Montserrat, sans-serif';
...
class App {
  constructor(
    container,
    {
      ...
      textColor = '#ffffff',
      borderRadius = 0,
      font = '600 20px Inter, Montserrat, sans-serif',
      scrollSpeed = 2,
      scrollEase = 0.05
    } = {}
  ) {
    ...
    try {
      ...
      console.warn('CircularGallery WebGL initialization skipped:', err);
    }
  }
  createRenderer() {
  ...
  addEventListeners() {
    ...
    // Only capture interaction inside container
    if (this.container) {
    ...
    // Safety release listeners on window in case user releases pointer outside container
...
const CircularGallery = forwardRef(function CircularGallery({
  ...
  textColor = '#ffffff',
  borderRadius = 0.05,
  font = '600 18px Inter, Montserrat, sans-serif',
  fontUrl = undefined,
  scrollSpeed = 2,
  scrollEase = 0.05,
  className = ''
  ...
  useImperativeHandle(ref, () => ({
    next: () => {
      if (appRef.current) {
        appRef.current.scroll.target += appRef.current.scrollSpeed * 6;
